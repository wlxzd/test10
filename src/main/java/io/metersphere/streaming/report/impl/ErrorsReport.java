...
public class ErrorsReport extends AbstractReport {
    ...
    public void execute() {
        ...
        errors.forEach(e -> {
            e.setPercentOfErrors(format3.format(new BigDecimal(e.getPercentOfErrors())));
            e.setPercentOfAllSamples(format3.format(new BigDecimal(e.getPercentOfAllSamples())));
        });
